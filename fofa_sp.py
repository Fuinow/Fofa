...
#       print self.temp_rule
        ...
        for item in items:
            ...
            domain = domain.find("a", target="_blank").get("href")
            print("[+] Get IP: %s"%domain)

            self.temp_host.add(ip)
            self.host_list.add(domain)
...
a = Fofa("app=\"Squid\" && body=\"Missing URL\"")
